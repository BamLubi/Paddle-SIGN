...
class L0_SIGN(nn.Layer):
    def __init__(self, pred_edges, n_feature, dim, hidden_layer, l0_para, batch_size):
...
class SIGN(nn.Layer):
    def __init__(self, dim, hidden_layer, aggr_func="mean"):
        ...
        ## 设置初始化权重
        self.lin1 = nn.Linear(dim, hidden_layer, weight_attr=paddle.nn.initializer.KaimingUniform())
        self.lin2 = nn.Linear(hidden_layer, dim, weight_attr=paddle.nn.initializer.KaimingUniform())
