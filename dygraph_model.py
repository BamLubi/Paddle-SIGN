...
class DygraphModel():
    ...
    def create_model(self, config):
        ...
        model = L0_SIGN(pred_edges, n_feature, dim, hidden_layer, l0_para, batch_size)
    ...
    def create_loss(self, output, label, l0_penaty, l2_penaty, l0_weight, l2_weight):
    ...
    def train_forward(self, dy_model, metrics_list, batch_data, config):
        ...
        loss = self.create_loss(output, labels, l0_penaty, l2_penaty, l0_weight, l2_weight)
        # update metrics
        predictions = np.vstack(output)
        labels = np.vstack(labels)
        labels = labels[:,1].reshape((-1,1))
        metrics_list[0].update(preds=predictions, labels=labels)
        correct = metrics_list[1].compute(paddle.to_tensor(predictions), paddle.to_tensor(labels))
