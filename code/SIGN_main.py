...
parser.add_argument('--n_epoch', type=int, default=500, help='the number of epochs 500')
...
a = int(len(dataset)* 0.7 * 0.5)
b = int(len(dataset)* 0.85 * 0.5)
c = int(len(dataset) * 0.5)
train_dataset = dataset[:a]
test_dataset = dataset[a:b]
val_dataset = dataset[b:c]
